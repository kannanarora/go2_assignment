...
class WanderNode(Node):
    def __init__(self):
        ...
        self.active_behaviour_topic = self.declare_parameter(
            "active_behaviour_topic", "/active_behaviour"
        ).value
        ...
        self.create_subscription(
            String,
            self.active_behaviour_topic,
            self._on_active_behaviour,
            10,
        )
        self.create_subscription(
            Bool,
        ...
        self.get_logger().info(
            "WanderNode gated on %s=wander, publishing velocity to %s "
            "(avoid<%.2fm, clear>%.2fm)"
            % (
                self.active_behaviour_topic,
                self.cmd_vel_topic,
    ...
    def _on_active_behaviour(self, msg: String):
        wander_active = msg.data.strip() == "wander"
        if wander_active == self._wander_active:
            return

        self._wander_active = wander_active
        if wander_active:
    ...
    def _on_safety(self, msg: Bool):
        if msg.data == self._safety_active:
            return

        self._safety_active = msg.data
        if self._safety_active:
            self.get_logger().info("Safety active — wander paused")
            self.publish_move(0.0, 0.0)

    def _is_enabled(self) -> bool:
        return self._wander_active and not self._safety_active
    ...
    def tick(self):
        ...
        if not self._startup_sent and self.startup_command:
